...
### COMPUTER DATA ###
computer_available_to_fire_set = {(a, b) for a in range(16, 26) for b in range(1, 11)}
    ...
    def draw_grid(self):
        ...
        for i in range(11):
            ...
            pygame.draw.line(
                ...
                (left_margin + (10 + self.offset) * block_size, upper_margin + i * block_size),
                1,
            )
            # Vertical lines
            pygame.draw.line(
                ...
                (left_margin + (i + self.offset) * block_size, upper_margin + 10 * block_size),
                1,
            )
...
class Button:
    def __init__(self, x_offset, button_title, message_to_show):
        ...
        self.rect_for_button_title = (
            self.x_start + self.button_width / 2 - self.title_width / 2,
    ...
    def print_message_for_button(self):
        ...
        if self.message:
            ...
            self.rect_for_message = (
                self.x_start / 2 - self.message_width / 2,
                self.y_start + self.button_height / 2 - self.message_height / 2,
            )
...
def check_hit_or_miss(
    fired_block, opponents_ships_list, computer_turn, opponents_ships_list_original_copy, opponents_ships_set
):
    # if computer_turn:
...
def main():
    ...
    rect_for_message_and_buttons = (0, upper_margin + 11 * block_size, size[0], 5 * block_size)

    message_rect_drawing_ships = (
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if human_ships_to_draw:
                    ...
                    used_blocks_for_manual_drawing = restore_used_blocks(deleted_ship, used_blocks_for_manual_drawing)
            ...
            # drawing is crucial here - otherwise it counts the previouc click up after clicking on MANUAL button!
                ...
                if (
                    ...
                    if start_block[0] == end_block[0] and (end_block[1] - start_block[1]) < 4:
                        ...
                        show_messages_at_rect_center("SHIP IS TOO LARGE! Try again!", message_rect_drawing_ships)
                        # print("SHIP IS TOO LARGE! Try again!")
                else:
                    show_messages_at_rect_center("SHIP IS BEYOND YOUR GRID! Try again!", message_rect_drawing_ships)
                ...
                if temp_ship:
                    ...
                    if ship_is_valid(temp_ship_set, used_blocks_for_manual_drawing):
                        ...
                        show_messages_at_rect_center("SHIPS ARE TOUCHING! Try again", message_rect_drawing_ships)
    ...
    while not game_over:
        ...
        for event in pygame.event.get():
            ...
            # Player shoots and is checked for hit or miss
                ...
                if (left_margin < x < left_margin + 10 * block_size) and (
                    ...
                    show_messages_at_rect_center("Your shot is outside of grid! Try again", message_rect_computer)
        ...
        if computer_turn:
            ...
            last_rect = ((left_margin + 15 * block_size, upper_margin + 12 * block_size), (10 * block_size, block_size))
            # screen.fill((255, 0, 0), last_rect)
        else:
            last_rect = ((left_margin, upper_margin + 12 * block_size), (10 * block_size, block_size))
            # screen.fill((255, 0, 0), last_rect)

        if not computer.ships_set:
            last_rect = ((0, 0), (0, 0))
            show_messages_at_rect_center("YOU WON!", (0, 0, size[0], size[1]), game_over_font)
            # game_over = True
        if not human_ships_set:
            last_rect = ((0, 0), (0, 0))
            show_messages_at_rect_center("YOU LOST!", (0, 0, size[0], size[1]), game_over_font)
            # game_over = True
